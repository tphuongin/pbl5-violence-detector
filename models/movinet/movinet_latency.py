...
evaluate_latency.py
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Đo latency phát hiện bạo lực của Jetson Violence Detection Client.
...
Workflow:
  ...
  3. Jetson phân tích video rồi đẩy kết quả về WS server của script
...
Chạy:
  python3 evaluate_latency.py --jetson 192.168.137.2 --config clips_config.json
...
Lưu ý cấu trúc thư mục:
  models/
    clips/
      vid1.mp4 ... vid6.mp4
    movinet/
      clips_config.json      ← path dùng "../vid1.mp4"
      evaluate_latency.py
"""
...
JETSON_PORT  = 8001              # HTTP port Jetson (aiohttp server)
...
class WsResultServer:
    ...
    Jetson sẽ kết nối vào đây với role=producer và stream
    frame payload + summary payload.
    ...
    async def _handler(self, ws):
        """Xử lý từng WebSocket connection từ Jetson."""
        # Path: /ws/video-analysis/{job_id}
        ...
        print(f"    [WS←Jetson] Kết nối: job={job_id[:8]}")
        try:
            async for raw in ws:
                pkt = json.loads(raw)
                ...
                if pkt.get("type") == "frame":
                    ...
                    print(f"    {icon} [{pct:5.1f}%] t={t_s:.2f}s  "
                          f"smooth={pkt.get('prob_smooth', 0):.3f}  {label}",
                          end="\r", flush=True)

                elif pkt.get("type") == "summary":
                    print()   # newline sau \r
                    await queue.put(pkt)
                    break     # summary là packet cuối, đóng kết nối
    ...
    async def start(self):
        ...
        print(f"[WS Server] Lắng nghe tại ws://0.0.0.0:{self._port}/ws/video-analysis/{{job_id}}")
...
def parse_args():
    p = argparse.ArgumentParser(description="Đánh giá latency Jetson Violence Detection")
...
async def upload_video(session: aiohttp.ClientSession,
    ...
    """Gửi video lên Jetson qua POST /analyze-video."""
...
async def evaluate_clip(session: aiohttp.ClientSession,
    ...
    print(f"\n{'─'*60}")
    print(f"[{clip_no}/{total}] {filename}")
    ...
    if not os.path.exists(path):
        ...
        return {"filename": filename, "gt_start_s": gt_start_s,
                "error": "File không tồn tại", "verdict": None,
    ...
    try:
        ...
        return {"filename": filename, "gt_start_s": gt_start_s,
                "job_id": job_id, "error": "Timeout",
                "verdict": None, "latency_s": None, "detected": False}
    finally:
        ws_server.unregister(job_id)
    ...
    duration  = summary.get("video_duration_s", 0)
    ...
    if segments:
        ...
        # gt_start_s = 0.0 → latency chính là detected_at_s
    ...
    print(f"  Xử lý         : {proc_time:.2f}s / {duration:.1f}s video "
          f"({proc_time/max(duration, 0.01)*100:.0f}% realtime)")
...
def print_summary_table(results: list):
    ...
    for r in results:
        ...
        lat_str = (f"+{latency:.3f}s" if latency is not None and latency >= 0
...
async def main():
    ...
    print(f"\n{'═'*60}")
    print(f"  Jetson Violence Detection — Latency Evaluator")
    print(f"  Jetson HTTP    : {jetson_url}")
    print(f"  Local IP       : {local_ip}  (Jetson sẽ gửi WS về đây)")
    print(f"  WS Server      : ws://0.0.0.0:{args.ws_port}/ws/video-analysis/{{job_id}}")
...
JETSON_HOST = "192.168.137.2"   # IP của Jetson Nano — đổi cho phù hợp
JETSON_PORT = 8001

# Danh sách video test + ground truth
# gt_start_s: giây bắt đầu bạo lực trong video (tính từ 0)
# gt_end_s  : giây kết thúc bạo lực (dùng để tính recall, có thể bỏ qua)
# Có thể khai báo trực tiếp ở đây hoặc dùng --config để load từ file JSON
...
def parse_args():
    ...
    p.add_argument("--config", default=None,
                   help="File JSON chứa danh sách clip + ground truth")
    p.add_argument("--output", default="latency_results.json",
                   help="File JSON lưu kết quả (mặc định: latency_results.json)")
    p.add_argument("--ws-backend", default=None,
                   help="Host backend nhận WebSocket kết quả video analysis "
                        "(mặc định: dùng --jetson)")
    return p.parse_args()
...
async def receive_summary(ws_url: str, job_id: str,
                          timeout: float = 300.0) -> dict:
    """
    Kết nối WebSocket và đợi nhận packet type='summary'.
    In tiến trình từng frame_payload trong lúc chờ.
    Timeout sau `timeout` giây.
    """
    deadline = time.time() + timeout
    async with websockets.connect(ws_url) as ws:
        print(f"    [WS] Kết nối OK, đang nhận kết quả …")
        async for raw in ws:
            if time.time() > deadline:
                raise TimeoutError(f"Timeout sau {timeout}s — không nhận được summary")
    ...
    raise RuntimeError("WebSocket đóng trước khi nhận được summary")
...
async def evaluate_clip(session: aiohttp.ClientSession,
    ...
    path       = clip["path"]
    gt_start_s = clip["gt_start_s"]
    gt_end_s   = clip.get("gt_end_s", None)
    note       = clip.get("note", "")
    filename   = os.path.basename(path)

    print(f"\n{'─'*60}")
    print(f"[{clip_no}/{total}] {filename}")
    print(f"  Ground truth  : bạo lực bắt đầu @ {gt_start_s:.2f}s")
    if gt_end_s:
        print(f"  Ground truth  : kết thúc @ {gt_end_s:.2f}s  "
              f"(kéo dài {gt_end_s - gt_start_s:.1f}s)")
    if note:
        print(f"  Ghi chú       : {note}")
    ...
    try:
        summary = await receive_summary(ws_url, job_id, timeout=300.0)
    except Exception as e:
        print(f"  ❌ Nhận kết quả lỗi: {e}")
        return {
            "filename":  filename,
            "gt_start_s": gt_start_s,
            "job_id":    job_id,
    ...
    # ── Tính latency ──────────────────────────────────────
    # latency = thời điểm phát hiện bạo lực đầu tiên - ground truth start
    # Âm = phát hiện sớm hơn GT (có thể do cooldown/EMA lan sang)
    ...
    print(f"\n  ── Latency ───────────────────────────────────")
    if detected:
        print(f"  GT bạo lực bắt đầu : {gt_start_s:.2f}s")
        print(f"  Phát hiện đầu tiên : {detected_at_s:.2f}s")
        if latency_s >= 0:
    ...
    print(f"  Thời gian phân tích: {proc_time:.2f}s  "
          f"(video {duration_s:.1f}s, "
          f"tỉ lệ {proc_time/max(duration_s,0.01)*100:.0f}% realtime)")
...
def print_summary_table(results: list):
    """In bảng tổng hợp tất cả kết quả."""

    rows = []
    for r in results:
        if r.get("error"):
            rows.append([
                r["filename"], "—", "—", "ERROR", r["error"][:30],
            ])
            continue
        ...
        rows.append([
            r["filename"],
            f"{r.get('gt_start_s', '?'):.2f}s",
            f"{r.get('detected_at_s', '—'):.2f}s" if r.get('detected_at_s') is not None else "—",
            lat_str,
            r.get("verdict", "?"),
        ])
    ...
    print(f"  Bỏ sót          : {len(missed)}")
    print(f"  Lỗi             : {len(errors)}")
    print(f"\n  Latency (chỉ clip phát hiện được):")
    print(f"    Trung bình    : {avg_lat:+.3f}s")
    print(f"    Nhỏ nhất      : {min_lat:+.3f}s")
    print(f"    Lớn nhất      : {max_lat:+.3f}s")
    print(f"    Độ lệch chuẩn : {std_lat:.3f}s")
...
async def main():
    ...
    jetson_url = f"http://{args.jetson}:{args.port}"
    # WebSocket kết quả video analysis — Jetson đẩy lên backend
    # Nếu không có backend riêng, giả sử Jetson cũng đóng vai trò nhận WS
    ws_backend = args.ws_backend or args.jetson
    ws_url_template = (
        f"ws://{ws_backend}:{8000}/ws/video-analysis/{{job_id}}"
    )

    # Load clips
    if args.config:
        clips = load_clips_from_config(args.config)
    else:
        clips = TEST_CLIPS
        print(f"[Config] Dùng TEST_CLIPS mặc định ({len(clips)} clip)")
        print(f"         Chỉnh sửa TEST_CLIPS trong file hoặc dùng --config để load từ JSON")

    if not clips:
        print("❌ Không có clip nào để đánh giá.")
        sys.exit(1)

    print(f"\n{'═'*60}")
    print(f"  Jetson Violence Detection — Latency Evaluator")
    print(f"  Jetson HTTP  : {jetson_url}")
    print(f"  WS backend   : {ws_url_template}")
    print(f"  Tổng clip    : {len(clips)}")
    print('═'*60)
    ...
    async with aiohttp.ClientSession() as session:
        for i, clip in enumerate(clips, 1):
            result = await evaluate_clip(
                session, jetson_url, ws_url_template,
                ws_backend, clip, i, len(clips)
            )
            results.append(result)
            # Nghỉ 2s giữa các clip để Jetson reset model state
            if i < len(clips):
                print(f"\n  Chờ 2s trước clip tiếp theo …")
